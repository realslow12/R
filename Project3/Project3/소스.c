...
void reset(void) {
	...
	for (i = 0; i < length; i++) { //뱀 몸통값 입력 
		x[i] = MAP_WIDTH / 2 + i;
		y[i] = MAP_HEIGHT / 2;
		gotoxy(MAP_X + x[i], MAP_Y + y[i], "♡");                     //아이콘 변경
	}
	gotoxy(MAP_X + x[0], MAP_Y + y[0], "♥"); //뱀 머리 그림            // 아이콘 변경
	food(); // food 생성  
}
...
void move(int dir) {
	...
	if (x[0] == food_x1 && y[0] == food_y1) {                     //장애물 과 충돌했을 경우 다이
		game_over();
		return;
	}
...
void food(void) {
	...
	while (1) {
		...
		food_x1 = (rand() % (MAP_WIDTH - 2)) + 1;                                           // 새로운 난수 추가  좌표값에 넣음
		food_y1 = (rand() % (MAP_HEIGHT - 2)) + 1;
		...
		for (i = 0; i < length; i++) { //food가 뱀 몸통과 겹치는지 확인  
			...
			if (food_x1 == x[i] && food_y1 == y[i]) {                                         // 장해물 먹이 생성
				food_crush_on = 1; //겹치면 food_crush_on 를 on 
				r++;
				break;
			}
		...
		speed -= -3; //속도 증가 
		gotoxy(MAP_X + food_x1, MAP_Y + food_y1, "★");										//장애물 먹이 생성 
